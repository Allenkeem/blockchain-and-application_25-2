...
contract CrowdFunding {
    ...
    constructor(uint _duration, uint _goalAmount) {
        ...
        status = "Funding";     // Funding으로 상태 설정
        ended = false;          // 종료 여부: False
    ...
    function fund() public payable {
        ...
        require(block.timestamp < deadline, "DEADLINE PASSED"); // 마감시간과 비교
        require(msg.value > 0, "NEED TO SEND OVER 0 ETH");   // 음수 방지
    ...
    function checkGoalReached() public onlyOwner {           // Owner만 실행 가능
        require(!ended, "Already ended");                    // ended가 false일때만 한번에 한해 실행 가능
        require(block.timestamp >= deadline, "Deadline not reached yet"); // 마감 시간 이후에 실행 가능 

        // 상황1) 목표 금액 이상 모인 경우: 소유자에게 전액 송금
        if (totalAmount >= goalAmount) {
            status = "Campaign Succeeded";                   // 모금 성공으로 상태 변경
            (bool ok, ) = payable(owner).call{value: totalAmount}("");  // Owner에게 송금
            require(ok, "Transfer to owner failed");
        } 
        // 2) 목표 금액 미달: 투자자들에게 환불
        else {
            status = "Campaign Failed";                      // 모금 실패로 상태 변경
            for (uint i = 0; i < numInvestors; i++) {
                (bool ok, ) = payable(investors[i].addr).call{value: investors[i].amount}("");  // 투자자에게 환불
    ...
    // 투자자 주소 목록 조회 함수
    function getInvestors() public view returns (address[] memory) {    // 모든 투자자의 주소를 배열 형태로 반환
