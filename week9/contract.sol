...
contract NameRegistry {
    ...
    // 컨트랙트 정보를 저장하는 구조체
    struct ContractInfo {
        address contractOwner;      // 이 이름에 대해 등록 권한을 가진 주소(오너)
        address contractAddress;    // 실제 컨트랙트 주소
        string description;         // 컨트랙트 설명
    }

    // 현재까지 등록된 컨트랙트 개수
    uint public numContracts;

    // 이름(string) → ContractInfo 매핑
    ...
    event ContractRegistered(
        ...
        address indexed owner,         // 컨트랙트 오너 주소
    ...
    event ContractDeleted(
        ...
        address indexed owner,         // 삭제 당시 오너
    ...
    event ContractUpdated(
        ...
        address indexed owner,         // 변경 후 기준 오너
        address contractAddress,       // 현재 컨트랙트 주소
        string description             // 현재 설명
    );

    // 특정 이름에 대한 오너만 함수 호출을 허용하는 제어자
    ...
    function registerContract(
        ...
        public
    {
        // 주소가 0이 아닌 경우에만 유효한 등록으로 처리
        if (_contractAddress != address(0)) {
            // msg.sender를 오너로 하여 정보 저장
    ...
    // 컨트랙트 등록 해제 함수
    // onlyOwner 제어자 덕분에 해당 이름의 오너만 삭제 가능
